function dbExtract(dataDir, tDir, flatten, skip )
...
for s=1:length(setIds)
  for v=1:length(vidIds{s})
    fprintf('\nSet %d/%d - vid %d/%d', setIds(s),setIds(length(setIds)), vidIds{s}(v), vidIds{s}(length(vidIds{s})));
    % load ground truth
    ...
    fs=cell(1,n); for i=1:n, fs{i}=['I' int2str2(i-1,5)]; end
    % extract images
    td=[tDir '/' name '/images/' post]; if(~exist(td,'dir')), mkdir(td); end
    sr=seqIo([pth '/' name '.seq'],'reader'); info=sr.getinfo();
    ...
    td=[tDir '/' name '/annotations/' post]; if(~exist(td,'dir')), mkdir(td); end
